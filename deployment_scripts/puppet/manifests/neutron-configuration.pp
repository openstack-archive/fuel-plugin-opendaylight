...
if $use_neutron {
  ...
  if $odl['enable_l3_odl'] {
    $ext_interface = odl_get_external_interface()
    $service_plugins        = [
  ...
  if $segmentation_type == 'vlan' {
    ...
    if $odl['enable_l3_odl'] {
      exec { 'ovs-set-provider-mapping':
        ...
        path    => '/usr/bin',
        before  => Exec['ovs-set-tunnel-endpoint'],
      }
    }
  ...
  if $odl['enable_l3_odl'] {
    exec { 'ovs-add-ext-interface':
      command => "sleep 15 && ovs-vsctl --may-exist add-port br-ex ${ext_interface}",
      path    => '/bin:/usr/bin',
      require => Exec['ovs-set-provider-mapping'],
    }
  }
